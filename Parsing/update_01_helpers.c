...
/*   Updated: 2023/08/05 01:27:45 by nabboune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_update_01_in_out_core(t_dic **ptr, char *var, t_iterators *itr)
{
	if ((*ptr)->next->value[itr->i + itr->j + 1] == '?')
		(*ptr)->next->value = ft_replace_str((*ptr)->next->value, ft_expand("?\0"),
											itr->i, itr->i + 1);
	else
	{
		while ((*ptr)->next->value[itr->i + itr->j + 1]
			&& ft_is_delimiter((*ptr)->next->value[itr->i + itr->j + 1]) != SPACE
			&& ft_is_delimiter((*ptr)->next->value[itr->i + itr->j + 1]) != DQUOTE)
			itr->j++;
		var = ft_substr((*ptr)->next->value, itr->i + 1, itr->j, 1);
		(*ptr)->next->value = ft_replace_str((*ptr)->next->value, ft_expand(var),
											itr->i, itr->i + itr->j);
	}
}
...
void	ft_update_01_in_out(t_dic **ptr, char *var, t_iterators *itr)
{
	...
	if ((*ptr)->next)
	{
		...
		{
			itr->j = 0;
			if ((*ptr)->next->value[itr->i] == '$' && (*ptr)->next->value[itr->i + itr->j + 1])
...
void	ft_update_01_dquote(t_dic **ptr, char *var, t_iterators *itr)
{
	...
	if ((*ptr)->next && (*ptr)->next->key == CMD)
	{
		...
		{
			itr->j = 0;
			if ((*ptr)->next->value[itr->i] == '$' && (*ptr)->next->value[itr->i + 1]
				&& (*ptr)->next->value[itr->i + 1] != '$')
			{
