...
int	ft_nodelquo(t_dic **dic, char *input, t_iterators *itr, int *pipe)
{
	...
	{
		while (input[itr->i + itr->j]
			&& !ft_is_delimiter(input[itr->i + itr->j]))
			itr->j++;
		ft_addpage_back(dic,
			ft_pagenew(CMD, ft_substr(input, itr->i, itr->j, 1), *pipe));
		itr->j--;
	}
	return (itr->j);
}
...
int	ft_delimiter_v00(t_dic **dic, char *input, t_iterators *itr, int *pipe)
{
	...
	else if (input[itr->i] && ft_is_delimiter(input[itr->i]) == SPACES)
		ft_addpage_back(dic, ft_pagenew(SPACES, " ", *pipe));
